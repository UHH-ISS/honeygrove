...
class IMAPService(ServiceBaseModel):
    def __init__(self):
        ...
        self._name = Config.imap.name
        self._port = Config.imap.port
        self._limiter = Limiter(self._fService, Config.imap.name, Config.imap.connections_per_host)
...
class IMAPProtocol(Protocol, policies.TimeoutMixin):
    def __init__(self):
        ...
        mm = Config.imap.authentication_methods
        for m in mm:
            if (mm[m]):
